...
let innderhtml = ''
let oneitem = items.forEach((item) => {
    innderhtml += `
...
itemcontainer.innerHTML = innderhtml
}


let cart = localStorage.getItem("cart") ? JSON.parse(localStorage.getItem('cart')) : []
let bagItem = document.querySelector(".bagcount")
if (cart.length > 0) {
...
function chekout() {
    ...
    let checkyitems = items.filter((item)=>{
        console.log('Checking item:', item.id, 'against cart id:', cart);
        return cart.includes(item.id); 
    })
    console.log(checkyitems)
